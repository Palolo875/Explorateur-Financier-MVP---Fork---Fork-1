...
import { useWeatherData } from '../hooks/useExternalAPI'
export function QuestionScreen() {
  const [question, setQuestion] = useState('')
  const { setUserQuestion } = useFinance()
  const { addQuestionToHistory, questionHistory, userCity, setUserCity } =
    useFinanceStore()
  const navigate = useNavigate()
  const { themeColors } = useTheme()
  const [suggestions] = useState([
    'Comment optimiser mon budget mensuel ?',
    "Puis-je m'acheter une voiture électrique ?",
    'Comment réduire mes dépenses fixes ?',
    "Quel est mon potentiel d'investissement ?",
  ...
  const { data: weatherData } = useWeatherData(userCity)
  const renderWeatherWidget = () => {
    if (!weatherData) return null
    return (
      <div className="mb-4 bg-black/20 rounded-lg p-3 flex items-center">
        <img
  ...
  return (
    ...
    >
      <div className="mb-8 text-center">
        ...
        {quote && (
          <motion.div
            className="italic text-base text-gray-300 mb-4 flex items-center justify-center"
            initial={{
              y: -10,
              opacity: 0,
            }}
        ...
        >
          {suggestions.map((suggestion, index) => (
            <button
              ...
              className={`text-sm px-3 py-1.5 rounded-full border ${themeColors.border} hover:bg-white/10 transition-colors duration-200`}
            >
              {suggestion}
            </button>
          ))}
        ...
        <form onSubmit={handleSubmit}>
          <div className="relative mb-6">
            ...
            <input
              ...
              className={`w-full bg-black/20 border ${themeColors.border} rounded-xl py-4 pl-14 pr-4 text-white text-lg placeholder-gray-500 focus:outline-none focus:ring-2 ring-offset-2 ring-offset-transparent focus:ring-indigo-500/50`}
              required
            />
          ...
          <div className="mb-4 bg-black/20 rounded-lg p-3">
            ...
            <input
              ...
              placeholder="Paris"
              className="w-full bg-black/30 border border-white/10 rounded-lg py-2 px-3 text-white"
            />
          </div>
          {/* Actualités en rapport avec la question */}
            ...
            <div className="mb-6 bg-black/20 rounded-lg p-4">
              ...
              <div className="space-y-2">
                {relatedNews.map((article, index) => (
                  <div
                    key={index}
                    className="text-sm hover:bg-black/20 p-2 rounded-lg transition-colors"
                  >
                    <a
                      href={article.url}
                      target="_blank"
          ...
          <div className="mb-6 bg-black/20 rounded-lg p-4">
            ...
            <div className="flex items-center">
              ...
              <input
                ...
                max="10"
                defaultValue="5"
                onChange={(e) => fetchAdviceForMood(parseInt(e.target.value))}
                className="flex-1 h-2 bg-black/40 rounded-lg appearance-none cursor-pointer"
                aria-label="Niveau de stress émotionnel"
              />
      ...
      {questionHistory.length > 0 && (
        <GlassCard className="p-6" animate>
          ...
          <div className="space-y-3 max-h-64 overflow-y-auto">
            {questionHistory.map((historyItem, index) => (
              <div
                key={index}
                className={`bg-black/20 rounded-xl p-4 cursor-pointer hover:bg-black/30 transition-colors flex items-center justify-between`}
