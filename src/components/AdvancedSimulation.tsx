  ...
  return <div className="w-full max-w-6xl mx-auto pb-20 p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <GlassCard className="lg:col-span-1 p-6" animate>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="number" name="incomeGrowth" value={params.incomeGrowth} onChange={handleParamChange} className="w-full bg-black/20 border border-white/10 rounded-lg py-2 px-3 text-white" />
            </div>
            <div>
              <label className="block text-sm mb-1">Réduction annuelle des dépenses (%)</label>
              <input type="number" name="expenseReduction" value={params.expenseReduction} onChange={handleParamChange} className="w-full bg-black/20 border border-white/10 rounded-lg py-2 px-3 text-white" />
            </div>
            <div>
              <label className="block text-sm mb-1">Taux d'épargne (%)</label>
              <input type="number" name="savingsRate" value={params.savingsRate} onChange={handleParamChange} className="w-full bg-black/20 border border-white/10 rounded-lg py-2 px-3 text-white" />
            </div>
            <div>
              <label className="block text-sm mb-1">Rendement annuel des investissements (%)</label>
              <input type="number" name="investmentReturn" value={params.investmentReturn} onChange={handleParamChange} className="w-full bg-black/20 border border-white/10 rounded-lg py-2 px-3 text-white" />
            </div>
            <div>
              <label className="block text-sm mb-1">Taux d'inflation annuel (%)</label>
              <input type="number" name="inflationRate" value={params.inflationRate} onChange={handleParamChange} className="w-full bg-black/20 border border-white/10 rounded-lg py-2 px-3 text-white" />
            </div>
            <div>
              <label className="block text-sm mb-1">Nombre d'années à simuler</label>
              <input type="number" name="years" value={params.years} onChange={handleParamChange} className="w-full bg-black/20 border border-white/10 rounded-lg py-2 px-3 text-white" />
            </div>
            <button onClick={handleRunSimulation} className={`w-full py-2 rounded-lg bg-gradient-to-r ${themeColors?.primary || 'from-indigo-500 to-purple-600'} hover:opacity-90 text-white`}>
        ...
        <GlassCard className="lg:col-span-2 p-6" animate>
          ...
          {result ? <div className="h-96">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={result.netWorth.map((value, index) => ({
              name: result.years[index],
              'Valeur Nette': value
            }))}>
